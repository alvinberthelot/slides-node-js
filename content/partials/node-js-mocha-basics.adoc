...
https://mochajs.org[Mocha] est une librairie pouvant exécuter des tests ("test runner") à la fois dans un environnement Node.js et dans un navigateur web.
...
La librairie Mocha est généralement exécutée via un environemment https://nodejs.org[Node.js], elle est donc installée comme un module npm.
...
La librairie Chai est exécutée sur le même environnement que Mocha donc via un environnement https://nodejs.org[Node.js], elle est donc installée comme un module npm.
...
Les tests seront regroupés logiquement avec la fonction +describe()+ et chaque test sera exécuté avec la fonction +it()+. Tous ayant un libellé.
